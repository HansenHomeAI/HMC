...
assert.match(source, /var TAP_DOT_DEFAULT_MAX_VISIBLE_DISTANCE = 0\.95;/, "Tap dots should default to a 30% shorter max visible distance so zoomed-out views stay uncluttered");
...
for (const caption of ["Main House", "Attached Shop and Studio/Guest Apt", "Guest/Caretaker Cabin", "Horse Barn"]) {
  assert.match(source, new RegExp(`caption: "${caption}"[\\s\\S]*?maxVisibleDistance: 0\\.95`), `${caption} should have the 30% shorter per-dot max visible distance`);
...
assert.doesNotMatch(source, /caption: "Mountain Lawn"/, "Incognito viewer should not restore the old sample Mountain Lawn tap dot");
...
assert.equal(tapDotMaxVisibleDistance({}), 0.95, "Missing max visible distance should use the 30% shorter uncluttered default");
